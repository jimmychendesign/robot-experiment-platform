...
domain: Robot Management / Experiment Schedule
owner: TBD
related_features: [F-003, F-004, F-005, F-008, F-009]
shared_contracts: [../../shared/scheduling-contract.md]
supersedes: null
superseded_by: PRD-005
...
关联归档 PRD：[PRD-001 实验需求方](./PRD-001-experiment-requester.md) · [PRD-003 实验员](./PRD-003-tester.md)  
替代版本：[PRD-005 实验管理者](../../active/PRD-005-experiment-manager.md)  
...
需求提交后先进入“待处理”。实验管理者打开需求并点击“开始处理”，锁定内容；随后触发脚本按需求 ID 创建并关联实验，系统自动检查完整性与排期结果，只有全部成功后才能确认“已排期”。失败、遗漏或关联异常保持“处理中”，由管理者查看原因并重试。

### 1.2 Goal

- 统一维护 Robot 工作时间、停机时间、状态、容量和 Tester 资格配置。
- 审批实验员请假，并触发对未执行实验的自动重新计算。
- 处理待处理需求、触发实验创建、检查创建结果，并通过“确认需求”进入测试执行。
...
flowchart TD
    ...
    E -- 是 --> G[确认需求]
    G --> H[需求进入已排期]
    H --> I[继续监控 Robot 与 Tester 排期]
    I --> J{处理哪类资源输入?}
    J -- Robot --> K[修改状态、工作/休息或额外停机时间]
    J -- Tester 映射 --> L[维护默认/备用 Tester 与操作资格]
    J -- 请假 --> M[查看请假申请]
    K --> N[系统重排受影响的未执行实验]
    L --> N
    M --> N
...
|---|---|---|
| 1 | 管理者进入运营页 | 系统展示待处理/处理中需求、今日指标、Robot 列表和排期 |
| 2 | 管理者打开待处理需求并开始处理 | 系统切换为“处理中”并锁定需求内容 |
| 3 | 管理者运行创建脚本 | 脚本携带需求 ID；实验创建后自动关联 |
| 4 | 创建或关联失败 | 需求保持“处理中”，管理员侧显示原因和重试；不退回需求方 |
| 5 | 全部创建成功 | “需求验证”显示“待验证”，“确认需求”可用；点击后需求进入“已排期” |
| 6 | 管理者进入实验员管理 | 系统展示 Tester 排期、Break 状态和待审批请假 |
| 7 | 管理者打开 Robot 详情 | 系统展示状态、容量、每日规则、默认/备用 Tester、停机时段和今日实验 |
| 8 | 管理者保存 Robot 或 Tester 配置 | 系统更新资源约束并识别受影响的未执行实验 |
| 9 | 管理者批准或拒绝请假 | 批准时重排覆盖时段内的未执行实验；拒绝时保留原排期 |
| 10 | 重排完成 | 系统将结果同步给需求方、管理者和相关实验员 |
...
| 字段 | 类型 | 必填 | 规则 |
|---|---|---:|---|
| 状态设置 | Select | 是 | 自动（根据排期）、已暂停、维护中；管理员不可直接设置运行中或空闲 |
...
暂停、维护或新增不可排时段后，系统降低对应容量并触发受影响实验的重排。选择“自动（根据排期）”后，系统重新计算可用容量，并按当前排期返回“运行中”或“空闲”。
...
管理员依次执行“开始处理”和“关联创建实验”。系统在创建后自动按需求 ID 关联实验。当前原型暂不实现具体的需求验证规则；创建成功后 Stepper 显示“需求验证 / 待验证”，并启用“确认需求”。管理员点击后直接进入“测试执行 / 待测试”。失败时操作区只显示明确原因和“重试”。

“实验需求队列”的状态列必须与需求方“我的需求”共用同一个 Requirement Status 映射，只显示“待处理、处理中、测试中、已完成”。管理员队列不得直接显示“已排期”“进行中”、内部 Stage / Status 或任一关联 Experiment 的 Status。当前优先级筛选“全部 / 紧急”保持不变；若未来增加 Requirement Status 筛选，必须使用“全部 / 待处理 / 处理中 / 测试中 / 已完成”。

需求详情 Stepper 使用共享的六阶段契约；创建、验证、DEBUG 和重新导出等具体处理状态不得替代固定 Stage 名称。完整映射引用[实验调度共享契约](../../shared/scheduling-contract.md#需求详情-stepper-阶段契约)。
...
```text
Given “需求验证”处于“待验证”且“确认需求”可用
When 管理员点击“确认需求”
Then Stepper 直接进入“测试执行 / 待测试”
And 不展示额外验证表单或验证步骤
```
...
Then 两个列表显示完全相同的“待处理、处理中、测试中、已完成”之一
...
“自动（根据排期）”是管理设置，不是新的 Robot 业务状态。它用于清除“已暂停/维护中”的人工覆盖，恢复后由系统输出运行中或空闲。
...
- BR-MGR-004：管理者只能设置已暂停或维护中；运行中与空闲必须由系统根据当前排期自动返回。
