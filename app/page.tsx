...
function StatusBadge({ value }: { value: string }) {
  const tone = ({
    ...
    "已安排": "scheduled", "已排期": "scheduled", "待执行": "scheduled",
...
export default function Home() {
  ...
  return (
    <div className="app-shell" ref={appRef}>
      ...
      <main id="main-content">
        ...
        <div className="content">
          ...
          {managerPage === "operations" && <section className="attention-overview" id="attention" aria-label="需要人工处理">
            <div className="issues">{attention.map((a, index) => { const IssueIcon = a.icon; return <article className="issue" key={`${a.kind}-${index}`} title={a.desc}><span className={`issue-icon ${a.tone}`}><IssueIcon aria-hidden="true" /></span><div className="issue-body"><small>{a.kind}</small><strong>{a.title}</strong></div><Button variant="secondary" size="sm" onClick={() => { const slot = getDispatchSchedule(platformRobotNames[4], 0, robotPool, testerBreaks, robotBlocks)[15]; setSelectedExperiment(scheduleSlotToExperiment(slot, 15)); }}>{a.action}</Button></article>; })}</div>
          </section>}
          ...
          <section className="panel capacity-panel">
              <div className="section-head"><div><h2>Robot 管理</h2><p>{selectedRobotNames.length ? `已选择 ${selectedRobotNames.length} 台 Robot，可通过更多操作批量设置` : "勾选 Robot 后，通过右上角更多操作进行批量设置。"}</p></div><div className="filters robot-more-wrap"><span className="filter-select-control"><select name="robot-status-filter" aria-label="按 Robot 状态筛选" value={statusFilter} onChange={e => setStatusFilter(e.target.value)}><option>全部状态</option><option>运行中</option><option>空闲</option><option>已暂停</option><option>维护中</option></select><ChevronDown aria-hidden="true" /></span><Tooltip content="更多操作" description={selectedRobotNames.length ? `已选择 ${selectedRobotNames.length} 台 Robot` : "请先勾选 Robot"}><button aria-label="更多操作" aria-haspopup="menu" aria-expanded={bulkMenuOpen} onClick={() => setBulkMenuOpen(open => !open)}><MoreHorizontal aria-hidden="true" /></button></Tooltip>{bulkMenuOpen && <div className="robot-more-menu" role="menu"><button role="menuitem" disabled={!selectedRobotNames.length} onClick={() => { setBulkMenuOpen(false); setBulkSettingsOpen(true); }}>批量设置{selectedRobotNames.length ? `（${selectedRobotNames.length}）` : ""}</button></div>}</div></div>
          ...
          <section className="panel schedule-panel requester-gantt manager-gantt">
            ...
            <div className="plan-constraint-note"><strong>今日排期 · 10:00–19:00</strong><span>全局平均用时 {globalRobotConfig.averageDuration} 分钟；全部 Robot {globalRobotConfig.breakStart}–{globalRobotConfig.breakEnd} 停用休息，其他不可排时段可在 Robot 详情中设置。</span></div><div className="request-gantt-body"><div className="request-gantt-head"><span>ROBOT / {calculatedDailyLimit} EXP CAPACITY</span><ScheduleTimeAxis /></div>{scheduleRows.map(row => <div className="request-gantt-row" key={row.name}><div className="request-robot-label"><span>R</span><div><strong>{row.name}</strong><small>{row.slots.filter(isBookedSlot).length}/{robotPool.find(robot => robot.name === row.name)?.capacity || calculatedDailyLimit} 已排 · {row.slots.find(isBookedSlot)?.tester || "待分配"}</small></div></div><div className="request-slot-track">{row.slots.map((slot, i) => <button key={slot.id} title={`${slotTimeLabel(i)} · ${slot.sub} · ${slot.policy} · ${slot.tester}${slot.constraint ? ` · ${slot.constraint}` : ""}`} className={`${slot.status} ${slot.blocked ? "blocked" : ""}`} onClick={() => slot.available || slot.blocked ? undefined : setSelectedExperiment(scheduleSlotToExperiment(slot, i))}><span>{slot.sub}</span><small>{slot.available ? slotTimeLabel(i) : slot.policy.replace(" Policy", "")}</small><em>{slot.available ? `约 ${globalRobotConfig.averageDuration} 分钟` : slot.tester}</em></button>)}</div></div>)}</div>
          </section>
      ...
      {(selectedRobot || selectedExperiment) && <div className="overlay" onMouseDown={() => { setSelectedRobot(null); setSelectedExperiment(null); setDrawerParentRobot(null); }}><aside className="drawer robot-management-drawer" role="dialog" aria-modal="true" aria-label={selectedRobot ? `${selectedRobot.name} Robot 详情` : `${selectedExperiment?.id} 实验详情`} onMouseDown={e => e.stopPropagation()}><IconButton className="close" label="关闭详情" icon={<X aria-hidden="true" />} onClick={() => { setSelectedRobot(null); setSelectedExperiment(null); setDrawerParentRobot(null); }} /><div className="drawer-scroll-content">{selectedRobot ? <RobotManagementDrawerView robot={selectedRobot} blocks={robotBlocks[selectedRobot.name] || []} workConfig={globalRobotConfig} schedule={getDispatchSchedule(selectedRobot.name, 0, robotPool, testerBreaks, robotBlocks)} onSave={(status, defaultTester, backupTesters) => { updateRobotStatus(selectedRobot.name, status); updateRobotTesters(selectedRobot.name, defaultTester, backupTesters); }} onAddBlock={block => addRobotBlock(selectedRobot.name, block)} onExperiment={experiment => { setDrawerParentRobot(selectedRobot); setSelectedRobot(null); setSelectedExperiment(experiment); }} /> : selectedExperiment && <ExperimentDrawerWithRequest experiment={selectedExperiment} onAssignTester={tester => assignExperimentTester(selectedExperiment.id, tester)} onBack={drawerParentRobot ? () => { setSelectedExperiment(null); setSelectedRobot(drawerParentRobot); setDrawerParentRobot(null); } : undefined} />}</div></aside></div>}
      {selectedManagerRequest && <div className="overlay request-detail-overlay" onMouseDown={() => setSelectedManagerRequestId(null)}><aside className="drawer request-detail-drawer" role="dialog" aria-modal="true" aria-label={`${selectedManagerRequest.id} 需求详情`} onMouseDown={event => event.stopPropagation()}><IconButton className="close" label="关闭需求详情" icon={<X aria-hidden="true" />} onClick={() => setSelectedManagerRequestId(null)} /><div className="drawer-scroll-content"><RequestDetailDrawer request={selectedManagerRequest} /></div></aside></div>}

      {bulkSettingsOpen && <div className="modal-backdrop" onMouseDown={() => setBulkSettingsOpen(false)}>
        <div className="modal robot-bulk-modal" role="dialog" aria-modal="true" aria-labelledby="bulk-robot-title" aria-describedby="bulk-robot-description" onMouseDown={event => event.stopPropagation()}>
          <IconButton className="composer-close" label="关闭批量设置" icon={<X aria-hidden="true" />} onClick={() => setBulkSettingsOpen(false)} />
          <div className="modal-icon blue"><Settings2 aria-hidden="true" /></div>
          <h3 id="bulk-robot-title">批量设置 Robot</h3>
          <p id="bulk-robot-description">将配置应用到已选择的 {selectedRobotNames.length} 台 Robot。保存后系统重新计算每日容量和未执行实验。</p>
          ...
          <div className="bulk-modal-grid">
            <label><span>每日工作开始</span><input name="robot-work-start" autoComplete="off" type="time" value={globalRobotConfig.workStart} onChange={event => setGlobalRobotConfig(config => ({ ...config, workStart: event.target.value }))} /></label>
            <label><span>每日工作结束</span><input name="robot-work-end" autoComplete="off" type="time" value={globalRobotConfig.workEnd} onChange={event => setGlobalRobotConfig(config => ({ ...config, workEnd: event.target.value }))} /></label>
            <label><span>停用 / 休息开始</span><input name="robot-break-start" autoComplete="off" type="time" value={globalRobotConfig.breakStart} onChange={event => setGlobalRobotConfig(config => ({ ...config, breakStart: event.target.value }))} /></label>
            <label><span>停用 / 休息结束</span><input name="robot-break-end" autoComplete="off" type="time" value={globalRobotConfig.breakEnd} onChange={event => setGlobalRobotConfig(config => ({ ...config, breakEnd: event.target.value }))} /></label>
            <label className="wide"><span>平均实验时长</span><select name="average-experiment-duration" autoComplete="off" value={globalRobotConfig.averageDuration} onChange={event => setGlobalRobotConfig(config => ({ ...config, averageDuration: Number(event.target.value) }))}><option value={30}>30 分钟</option><option value={45}>45 分钟</option><option value={60}>60 分钟</option></select></label>
          </div>
          <div className="bulk-capacity-preview" aria-live="polite"><div className="bulk-capacity-result"><span>预计每日可排容量</span><strong>{calculatedDailyLimit}</strong><em>个实验 / Robot</em></div><div className="bulk-capacity-formula"><span>计算依据</span><p><strong>{globalWorkMinutes / 60} 小时</strong>工作跨度 − <strong>{globalBreakMinutes / 60} 小时</strong>停用时间</p><small>净可用时间按每个实验 {globalRobotConfig.averageDuration} 分钟折算</small></div></div>
          <div className="modal-actions"><Button variant="secondary" onClick={() => setBulkSettingsOpen(false)}>取消</Button><Button onClick={() => { applyGlobalRobotConfig(); setBulkSettingsOpen(false); }}>保存并应用</Button></div>
        </div>
      </div>}
...
function RequesterConsole({ requests, setRequests, robotPool, robotBlocks, testerBreaks, onAutoSchedule }: { requests: SharedRequest[]; setRequests: Dispatch<SetStateAction<SharedRequest[]>>; robotPool: Robot[]; robotBlocks: Record<string, RobotBlock[]>; testerBreaks: TesterBreak[]; onAutoSchedule: (request: SharedRequest) => void }) {
  const dialogReturnFocusRef = useRef<HTMLElement | null>(null);
  const dialogWasOpenRef = useRef(false);
...
function RequestDetailDrawer({ request }: { request: SharedRequest }) {
  const [activeTab, setActiveTab] = useState<"config" | "experiments">("config");
  const policies = request.policies?.length ? request.policies : [request.policy];
  const robotChoices = request.robotChoices?.length ? request.robotChoices : [request.robot];
  ...
  return <>
    ...
    {activeTab === "config" && <div className="request-progress">{["需求已提交", "管理员创建实验", "实验执行", "结果同步"].map((label, index) => <div className={index < flowStep ? "done" : index === flowStep ? "current" : "upcoming"} aria-current={index === flowStep ? "step" : undefined} key={label}><span>{index < flowStep ? <Check aria-hidden="true" /> : index + 1}</span><small>{label}</small></div>)}</div>}
    {activeTab === "config" ? <section className="request-detail-section request-config-panel" role="tabpanel">
      <div className="request-detail-heading"><h3>需求配置</h3><span className={`priority ${request.priority === "高" ? "high" : ""}`}>{request.priority}优先级</span></div>
...
function ModernRequestModal({ requestCount, setRequests: _setRequests, robotPool, robotBlocks, testerBreaks, onAutoSchedule, onClose }: { requestCount: number; setRequests: React.Dispatch<React.SetStateAction<SharedRequest[]>>; robotPool: Robot[]; robotBlocks: Record<string, RobotBlock[]>; testerBreaks: TesterBreak[]; onAutoSchedule: (request: SharedRequest) => void; onClose: () => void }) {
  ...
  return <div className="modal-backdrop modern-request-backdrop"><form className={`modern-request-modal ${selectedRobots.length ? "calendar-open" : ""}`} role="dialog" aria-modal="true" aria-labelledby="new-request-title" aria-describedby="new-request-description" onSubmit={submit}>
    <header className="modern-request-head"><div className="modal-icon blue"><FlaskConical aria-hidden="true" /></div><div><h3 id="new-request-title">提交实验需求</h3><p id="new-request-description">提交后系统将自动创建实验，并匹配 Robot、合格 Tester 与最近可用时间。</p></div><IconButton type="button" label="关闭" icon={<X aria-hidden="true" />} onClick={onClose} /></header>
...
function ExperimentDrawerWithRequest({ experiment: e, onAssignTester, onBack }: { experiment: Experiment; onAssignTester: (tester: string) => void; onBack?: () => void }) {
  const [tester, setTester] = useState(sharedTesterNames[0]);
  const needsTester = e.tester === "待分配" || e.status === "冲突";
  return <>
    <header className="experiment-drawer-head">{onBack && <Button className="drawer-back" variant="tertiary" size="sm" leadingIcon={<ArrowLeft aria-hidden="true" />} onClick={onBack}>返回 Robot 详情</Button>}<div className="drawer-kicker">实验详情 · {e.requestId || "关联需求"}</div><h2>{e.id}</h2><div className="drawer-title"><strong>{e.name}</strong><span className={`priority ${e.priority === "高" ? "high" : ""}`}>{e.priority}优先级</span></div><StatusBadge value={needsTester ? "等待资源" : e.status} /></header>
